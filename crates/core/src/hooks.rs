...
/// Run a package's build commands. Runs after version bump, before commit —
/// output artifacts must match the declared `artifacts` globs (sr validates
/// this before tagging).
pub fn run_build(commands: &[String], env: &[(&str, &str)]) -> Result<(), ReleaseError> {
    run_commands("build", commands, env)
}
